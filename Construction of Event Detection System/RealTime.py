...
MODELS = {
    "CRF": {"precision": 56.4, "recall": 59.6, "f1": 53.1},
    "BiLSTM-CRF": {"precision": 54.8, "recall": 54.7, "f1": 50.1},
    "PhoBERT": {"precision": 67.1, "recall": 60.4, "f1": 70.1}
}
...
def simulate_processing(text_input, model_choice, progress=gr.Progress()):
    ...
    model_precision = MODELS[model_choice]["precision"] / 100  # Convert percentage to decimal
    confidence_primary = model_precision * (0.8 + 0.4 * random.random())  # Base precision with some randomness
    
    # Ensure confidence is between 0 and 1
    confidence_primary = min(max(confidence_primary, 0.55), 0.85)
    
    # Calculate remaining confidence to distribute
    remaining = 1.0 - confidence_primary
    
    # Create confidence scores for all classes
    confidences = {}
    
    # First, assign random proportions to the other event types
    other_events = [et for et in EVENT_TYPES if et != event_type]
    prop1 = random.random()
    prop2 = 1.0 - prop1
    
    # Calculate actual values with proportions
    confidences[other_events[0]] = remaining * prop1
    confidences[other_events[1]] = remaining * prop2
    
    # Add the main event type with highest confidence
    confidences[event_type] = confidence_primary
    
    # Verify the sum is exactly 1.0 (100%)
    total = sum(confidences.values())
    if abs(total - 1.0) > 1e-10:  # Check if not exactly 1.0 (allowing for floating point precision)
        # Normalize values to ensure they sum to exactly 1.0
        for et in confidences:
            confidences[et] = confidences[et] / total
    ...
    # Add the total to verify it equals 100%
    result_text += f"\nTotal: {sum(confidences.values()):.2%}"
